...
# available commands:
# Format:
#     cmd VID PID read/write [[GPIO#] [REGADDR]] [data_len] [data_value]
#
# GPIO:
#    GPIO#: 2, 3, 10, 11
#     READ: gpio 0x0424 0x2734 0x00 2
#    WRITE: gpio 0x0424 0x2734 0x01 3 0x01/0x00 (ON/OFF)
#
# POWER:
#  REGADDR: 0x3c00, 0x3c04, 0x3c08, 0x3c0c (USB Port 1, 2, 3, 4)
#     READ: register_rw 0x0424 0x2734 0x00 0x3c00 1
#    WRITE: register_rw 0x0424 0x2734 0x01 0x3c08 1 0x93/0x90 (ON/OFF)
#
...
help_msg () {
	echo "Command Syntax:"
	echo "	power-cycle.sh [usb_num] [repeat_count]"
...
led_onoff () {
	...
	on)
		flock -x 999
		STAT=$(/usr/sbin/gpio $VID $PID 0x01 $GPIOLED 0x01)
		flock -u 999
		;;
	off)
		flock -x 999
		STAT=$(/usr/sbin/gpio $VID $PID 0x01 $GPIOLED 0x00)
		flock -u 999
		;;
	*)
		;;
...
power_stat () {
	flock -x 999
	STAT=$(register_rw $VID $PID 0x00 $REGADDR 1 | grep "Register Read value is" | sed -E 's/(.*is\s(0x[0-9a-fA-F][0-9a-fA-F])\s.*)/\2/')
	flock -u 999
...
power_onoff () {
	...
	on)
		VAL=$(( ONOFF_STAT | 0x03 ))
		flock -x 999
		RESULT=$(register_rw $VID $PID 0x01 $REGADDR 1 $VAL)
		flock -u 999
		;;
	off)
		VAL=$(( ONOFF_STAT & 0xF0 ))
...
start_usb () {
	...
	sleep 10
	det-gpio.sh $1 $$ & DET_PID="$!"
}

#
# main entry
#
if [ -z "$1" ] || [ "$1" -lt 1 ] || [ "$1" -gt 4 ] || [ "$1" == "-h" ] || [ "$1" == "--help" ]; then
	help_msg
	exit 1
fi

if [ -n "$1" ] && [ -n "$2" ] && [ "$2" -lt 0 ]; then
	help_msg
	exit 1
fi

detected_gpio () {
	echo "detected gpio from USB$NUM"
}
...
init_time="$(date -u +%s)"
echo "===== DATE: $TODAY =====" >> $LOG
echo "power cycle test on USB$NUM" >> $LOG
